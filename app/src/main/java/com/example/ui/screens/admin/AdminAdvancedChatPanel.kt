...
fun AdminAdvancedChatPanel(
    ...
    with(state) {
        Column(
            ...
            Text("⚡ صلاحيات وتوجيه الدردشات المتقدمة", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("إعدادات التوجيه والدردشة الذكية:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("جميع صلاحيات الدردشة متقدمة ومفعلة افتراضياً.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
