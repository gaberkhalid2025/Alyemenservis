...
fun AdminComplaintsPanel(
    ...
    with(state) {
        Column(
            ...
            Text("⚠️ إدارة الشكاوى والبلاغات والتقارير", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)
            OutlinedTextField(
                value = complaintsSearchQueryState.value,
                onValueChange = { complaintsSearchQueryState.value = it },
                modifier = Modifier.fillMaxWidth(),
                placeholder = { Text("ابحث في الشكاوى...", color = Color.Gray, fontSize = 12.sp) },
                leadingIcon = { Icon(Icons.Default.Search, contentDescription = null, tint = Color.Gray) }
            )
            Text("لا توجد شكاوى جديدة حالياً.", color = Color.LightGray, fontSize = 11.sp)
        }
    }
}
