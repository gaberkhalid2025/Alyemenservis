...
@Composable
fun AdminBlockedPanel(
    viewModel: MainViewModel,
    themeColors: VisualThemePalette,
    state: AdminPanelState
) {
    if (!PermissionGuard.hasPermission(RoleManager.fromRoleString(viewModel.adminRole.value), "MANAGE_BLOCKED")) {
        ...
        Column(
            ...
            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("المستخدمون والحسابات المحظورة من النظام:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("لا توجد حسابات محظورة حالياً.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
