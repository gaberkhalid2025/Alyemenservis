...
@Composable
fun AdminDeletedPanel(
    viewModel: MainViewModel,
    themeColors: VisualThemePalette,
    state: AdminPanelState
) {
    if (!PermissionGuard.hasPermission(RoleManager.fromRoleString(viewModel.adminRole.value), "MANAGE_DELETED")) {
        ...
        Column(
            ...
            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("العناصر والخدمات المحذوفة مؤقتاً:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("سلة المحذوفات فارغة تماماً.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
