...
fun AdminCustomTabsPanel(
    ...
    with(state) {
        Column(
            ...
            Text("📑 تخصيص تبويبات الملفات الشخصية والخدمات", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("إدارة التبويبات المخصصة للملفات:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("لا توجد تبويبات مخصصة مضافة حالياً.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
