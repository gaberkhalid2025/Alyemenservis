...
fun AdminRolesAndPermissionsPanel(
    ...
    with(state) {
        Column(
            ...
            Text("📋 دليل وصلاحيات جميع المستخدمين والأدوار", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("أدوار وصلاحيات النظام:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("• المالك (OWNER): صلاحيات كاملة ومطلقة.", fontSize = 11.sp, color = Color.LightGray)
                    Text("• المدير (ADMIN): إدارة الأقسام والمشرفين والمحتوى.", fontSize = 11.sp, color = Color.LightGray)
                    Text("• المشرف (SUPERVISOR): مراجعة الطلبات والشكاوى.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
