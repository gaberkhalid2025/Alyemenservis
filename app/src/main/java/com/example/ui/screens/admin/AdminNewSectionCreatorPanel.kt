...
fun AdminNewSectionCreatorPanel(
    ...
    with(state) {
        Column(
            ...
            Text("➕ إضافة وإدارة الأقسام الديناميكية الجديدة", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("الأقسام النشطة في التطبيق:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("لا توجد أقسام إضافية حالياً.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
