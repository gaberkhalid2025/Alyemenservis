...
fun AdminBackupPanel(
    ...
    with(state) {
        Column(
            ...
            Text("💾 النسخ الاحتياطي والمزامنة واستعادة البيانات", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)
            ...
            Button(
                ...
                Text("مزامنة وتحديث البيانات من الخادم 🔄", color = Color.White)
            }

            OutlinedTextField(
                value = restoreJsonInputState.value,
                onValueChange = { restoreJsonInputState.value = it },
                label = { Text("أدخل بيانات الاستعادة (JSON)") },
                modifier = Modifier.fillMaxWidth()
            )
        }
    }
