...
fun AdminCallsPanel(
    ...
    if (!PermissionGuard.hasPermission(RoleManager.fromRoleString(viewModel.adminRole.value), "VIEW_CALLS")) {
    ...
    with(state) {
        Column(
            ...
            Text("📞 مراقبة المكالمات وسجلات الاتصال", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("سجل المكالمات والطلبات الهاتفية عبر التطبيق:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("لا توجد مكالمات مسجلة حديثاً في السجل.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
