...
fun AdminBookingRoutingPanel(
    ...
    with(state) {
        Column(
            ...
            Text("🔄 توجيه الحجوزات الذكي والتوزيع الآلي", fontSize = 14.sp, fontWeight = FontWeight.Bold, color = themeColors.accent)

            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = themeColors.surface)) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("قواعد توجيه الحجوزات لمزودي الخدمات:", fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
                    Text("التوجيه التلقائي مفعل حسب النطاق الجغرافي.", fontSize = 11.sp, color = Color.LightGray)
                }
            }
        }
    }
