...
class MainViewModel : ViewModel() {
    ...
    fun authenticateAdmin(context: android.content.Context, role: String, remember: Boolean) {
        ...
        val arabicRole = RoleManager.getArabicRoleName(RoleManager.fromRoleString(role))
        triggerNotification("🔓 تم تسجيل الدخول بنجاح بصلاحية: $arabicRole")
    ...
    fun authenticateAdmin(role: String) {
        ...
        val arabicRole = RoleManager.getArabicRoleName(RoleManager.fromRoleString(role))
        triggerNotification("🔓 تم تسجيل الدخول بنجاح بصلاحية: $arabicRole")
        if (_currentScreen.value != "ADMIN_PANEL") {
            _currentScreen.value = "ADMIN_PANEL"
        }
    }

    fun getCurrentUserRole(): UserRole {
        return RoleManager.fromRoleString(_adminRole.value)
    }

    fun canCurrentRoleAccessAdmin(): Boolean {
        return RoleManager.canAccessAdminFeatures(getCurrentUserRole())
    }
    ...
    // --- STORES MANAGEMENT ---
    fun addStore(name: String, description: String = "", phone: String = "", categoryId: String = "", cityId: String = "", workingHours: String = "9:00 AM - 10:00 PM", logoImage: String = "") {
        val newStore = StoreEntity(
            id = "store_" + java.util.UUID.randomUUID().toString().take(6),
            name = name,
        ...
        try {
            db.collection("stores").document(newStore.id).set(newStore)
        } catch (e: Exception) {}
        triggerNotification("🏪 تم إضافة المحل بنجاح: $name")
    }
    ...
    fun deleteStore(storeId: String) {
        ...
        triggerNotification("🗑️ تم حذف المحل بنجاح!")
    }
    ...
    fun addProperty(title: String, description: String = "", price: Double = 0.0, type: String = "sale", propertyType: String = "apartment", phone: String = "", cityId: String = "") {
        val newProp = PropertyEntity(
            id = "prop_" + java.util.UUID.randomUUID().toString().take(6),
            title = title,
            description = description,
            price = price,
            type = type,
        ...
        _properties.value = _properties.value + newProp
        try {
            db.collection("properties").document(newProp.id).set(newProp)
        } catch (e: Exception) {}
        triggerNotification("🏠 تم إضافة العقار بنجاح: $title")
    }
    ...
    fun togglePropertyBlock(propertyId: String) {
        _properties.value = _properties.value.map {
            if (it.id == propertyId) it.copy(isBlocked = !it.isBlocked) else it
        }
        triggerNotification("🔄 تم تحديث حالة حظر العقار")
    }

    // --- JOBS MANAGEMENT ---
    fun addJob(title: String, companyName: String = "", description: String = "", phone: String = "", salary: String = "", jobType: String = "دوام كامل", cityId: String = "") {
        val newJob = JobEntity(
            id = "job_" + java.util.UUID.randomUUID().toString().take(6),
            title = title,
        ...
        try {
            db.collection("jobs").document(newJob.id).set(newJob)
        } catch (e: Exception) {}
        triggerNotification("💼 تم إضافة الإعلان الوظيفي بنجاح: $title")
    }
    ...
    fun deleteJob(jobId: String) {
        ...
        triggerNotification("🗑️ تم حذف الوظيفة بنجاح!")
    }

    fun toggleJobBlock(jobId: String) {
        _jobs.value = _jobs.value.map {
            if (it.id == jobId) it.copy(isBlocked = !it.isBlocked) else it
        }
        triggerNotification("🔄 تم تحديث حالة حظر الإعلان الوظيفي")
    }

    fun toggleJobPin(jobId: String) {
        _jobs.value = _jobs.value.map {
            if (it.id == jobId) it.copy(isPinned = !it.isPinned) else it
        }
        triggerNotification("📌 تم تغيير حالة تثبيت الإعلان")
    }
    ...
    fun deleteSupervisor(supervisorId: String) {
        _supervisors.value = _supervisors.value.filterNot { it.id == supervisorId }
        try {
            db.collection("supervisors").document(supervisorId).delete()
        } catch (e: Exception) {}
        triggerNotification("🗑️ تم حذف المشرف بنجاح")
    }

    // --- SYSTEM WIPE & RESTORE ---
    fun wipeSelectedData(
        wipeProviders: Boolean = false,
        wipeBookings: Boolean = false,
        wipeChats: Boolean = false,
        wipeNotifs: Boolean = false,
        wipeReports: Boolean = false,
        wipeCategories: Boolean = false,
        wipeStores: Boolean = false,
        wipeProperties: Boolean = false,
        wipeJobs: Boolean = false
    ) {
        if (wipeProviders) _providers.value = emptyList()
        if (wipeBookings) _bookings.value = emptyList()
        if (wipeChats) _chatChannels.value = emptyList()
        if (wipeNotifs) _notifications.value = emptyList()
        if (wipeReports) _reports.value = emptyList()
        if (wipeCategories) _categories.value = emptyList()
        if (wipeStores) _stores.value = emptyList()
        if (wipeProperties) _properties.value = emptyList()
        if (wipeJobs) _jobs.value = emptyList()
        triggerNotification("🧹 تم تهيئة وتنظيف البيانات المحددة بنجاح!")
    }
