...
class RegistrationRepositoryImpl(
    ...
    private suspend fun sendAdminJoinNotification(requestId: String, applicantName: String, phone: String, type: String) {
        try {
            ...
            Log.e("RegistrationRepoImpl", "Failed to send admin notification", e)
        }
    }

    override suspend fun registerClient(client: RegistrationEntity.Client): AppResult<String> {
        return try {
            val cleanPhone = client.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب تسجيل قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }

            val id = UUID.randomUUID().toString()
            val hashedPassword = PasswordHasher.hash(client.passwordHash)
            ...
            Log.e("RegistrationRepoImpl", "Error registering client", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل العميل"))
        }
    }

    override suspend fun registerProvider(provider: RegistrationEntity.Provider): AppResult<String> {
        return try {
            val cleanPhone = provider.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب انضمام مهني قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(provider.passwordHash),
            ...
            Log.e("RegistrationRepoImpl", "Error registering provider", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل مقدم الخدمة"))
        }
    }

    override suspend fun registerStore(store: RegistrationEntity.Store): AppResult<String> {
        return try {
            val cleanPhone = store.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب انضمام متجر قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(store.passwordHash),
            ...
            Log.e("RegistrationRepoImpl", "Error registering store", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل المتجر"))
        }
    }

    override suspend fun registerRestaurant(restaurant: RegistrationEntity.Restaurant): AppResult<String> {
        return try {
            val cleanPhone = restaurant.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب انضمام مطعم قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(restaurant.passwordHash),
            ...
            Log.e("RegistrationRepoImpl", "Error registering restaurant", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل المطعم"))
        }
    }

    override suspend fun registerMedicalCenter(medical: RegistrationEntity.MedicalCenter): AppResult<String> {
        return try {
            val cleanPhone = medical.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب انضمام مركز طبي قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(medical.passwordHash),
            ...
            Log.e("RegistrationRepoImpl", "Error registering medical center", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل المركز الطبي"))
        }
    }

    override suspend fun registerProperty(property: RegistrationEntity.Property): AppResult<String> {
        return try {
            val cleanPhone = property.phone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد طلب إضافة عقار قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(property.passwordHash),
            ...
            Log.e("RegistrationRepoImpl", "Error registering property", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل العقار"))
        }
    }

    override suspend fun registerJob(job: RegistrationEntity.Job): AppResult<String> {
        return try {
            val cleanPhone = job.contactPhone.trim().replace(" ", "").replace("+", "")
            if (checkExistingPendingRequest(cleanPhone)) {
                return Result.failure(AppError.ConflictError("يوجد إعلان توظيف قيد المراجعة بالفعل لرقم الهاتف هذا"))
            }
            ...
            val request = JoinRequestEntity(
                ...
                phone = cleanPhone,
                passwordHash = PasswordHasher.hash(job.passwordHash),
                city = job.city.trim(),
                approvalStatus = "PENDING",
                submittedAt = System.currentTimeMillis(),
                createdAt = System.currentTimeMillis(),
            ...
            Log.e("RegistrationRepoImpl", "Error registering job", e)
            Result.failure(AppError.DatabaseError(e.localizedMessage ?: "فشل تسجيل الوظيفة"))
        }
    }
    ...
    suspend fun submitJoinRequest(request: JoinRequestEntity): Result<Unit> {
        return try {
            firestore.collection("join_requests").document(request.id.ifBlank { UUID.randomUUID().toString() }).set(request).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    fun observeAllJoinRequests(): Flow<List<JoinRequestEntity>> = callbackFlow {
        val listener = firestore.collection("join_requests")
            .addSnapshotListener { snapshot, _ ->
                if (snapshot != null) {
                    val list = snapshot.documents.mapNotNull { it.toObject(JoinRequestEntity::class.java)?.copy(id = it.id) }
                    trySend(list)
                }
            }
        awaitClose { listener.remove() }
    ...
    suspend fun approveJoinRequest(requestId: String, adminId: String): Result<Unit> {
        return try {
            firestore.collection("join_requests").document(requestId).update(mapOf("status" to "APPROVED", "approvalStatus" to "APPROVED")).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    suspend fun rejectJoinRequest(requestId: String, reason: String, adminId: String): Result<Unit> {
        return try {
            firestore.collection("join_requests").document(requestId).update(mapOf("status" to "REJECTED", "approvalStatus" to "REJECTED", "rejectionReason" to reason)).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
